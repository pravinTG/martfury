  ...
  class _OrderSummaryScreenState extends State<OrderSummaryScreen> {
    ...
    Future<void> _fetchWalletBalance() async {
      try {
        ...
        if (uid != null) {
          final balance = await _apiService.getUsableBalance(uid);
          if (mounted) {
            setState(() {
    ...
    Future<void> _placeOrder() async {
      ...
      try {
        ...
        if (_selectedPaymentMethod == 'razorpay') {
          ...
          final result = await _apiService.payFromWallet(
            userId: uid,
            billing: billing,
            products: lineItems,
          );
          ...
          orderId = (result['order_id'] ?? result['id'] ?? result['number'])?.toString() ?? '';
          if (orderId.isEmpty) {
            print('WALLET PAYMENT RESULT: $result');
            throw Exception('Wallet payment succeeded but no order ID was returned. API Response: $result');
          }
  ...
  Future<void> _clearCartAfterOrder() async {
    try {
      print('🧹 Clearing cart after successful order using removeFromCart API...');
      final removedPids = <String>{};

      // 1. Remove all items present in current screen's _cartItems
      final itemsToRemove = List<dynamic>.from(_cartItems);
      for (final item in itemsToRemove) {
        if (item is Map) {
          final pid = item['product_id']?.toString() ?? item['id']?.toString();
          if (pid != null && pid.isNotEmpty && pid != '0' && !removedPids.contains(pid)) {
            removedPids.add(pid);
            try {
              await _apiService.removeFromCart(productId: pid);
            } catch (e) {
              print('⚠️ Error removing product $pid from cart: $e');
            }
          }
        }
      }

      // 2. Fetch server cart to ensure no other items remain (e.g. from Buy Now mode or previous session)
      try {
        final serverCart = await _apiService.getCart();
        final serverItems = serverCart['cart_items'] ?? serverCart['items'] ?? serverCart['data'];
        if (serverItems is List && serverItems.isNotEmpty) {
            ...
            if (item is Map) {
              ...
              if (pid != null && pid.isNotEmpty && pid != '0' && !removedPids.contains(pid)) {
                ...
                try {
                  ...
                  print('⚠️ Error removing server product $pid from cart: $e');
                }
              }
            }
          }
        ...
        print('⚠️ Could not fetch server cart for cleanup: $e');
      }

      print('✅ Cart cleared via removeFromCart API.');
    } catch (e) {
      ...
      if (mounted) {
        setState(() {
          _localCartData = {
            ...
            'cart_total': '0.00',
            'total': '0.00',
            'cart_totals': {'total': '0.00', 'total_items': 0},
          };
        });
      }
    }
            ...
            RadioListTile<String>(
              title: Row(
                children: const [
                  ...
                  SizedBox(width: 10),
                  Expanded(child: Text('Razorpay (Cards, UPI, NetBanking)', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14))),
                ],
              ),
              value: 'razorpay',
            ...
            RadioListTile<String>(
              title: Row(
                children: const [
                  ...
                  SizedBox(width: 10),
                  Expanded(child: Text('Cash on Delivery (COD)', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14))),
                ],
              ),
              value: 'cod',
              groupValue: _selectedPaymentMethod,
